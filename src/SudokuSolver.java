...
public class SudokuSolver implements ISudokuSolver {
	...
	public void setValue(int col, int row, int value) {
		if(value > 0  && value < 10) // throw IllegalArgument("out of range");
	...
	public boolean solve() {
		...
		//FC
		ArrayList<Integer> Dtmp = FC(asn);
	
		if ( Dtmp == null ) return false; 
		
		for(int i = 0; i < Dtmp.size(); i++){
			setValue(GetColumn(i), GetRow(i), Dtmp.get(i));
		}
		
		return true;
	}
	
	int[][] p1 = new int[][]{{5,0,9,0,1,0,0,0,0},
							 {8,0,6,3,7,9,0,0,2},
							 {0,1,0,0,0,8,4,0,3},
							 {0,0,0,0,6,0,8,0,0},
							 {9,0,0,1,0,4,0,0,7},
	...
	public ArrayList FC(ArrayList<Integer> asn) {
		...
		if( !asn.contains(0) ) return asn;
		int X = asn.indexOf(0);
		ArrayList<ArrayList<Integer>> Dold = D;
		for(ArrayList<Integer> Dx: Dold){
			for(Integer V : Dx){
				if(AC_FC(X, V)){ // arc-consistency between X, V
					asn.set(X, V);
					ArrayList R = FC(asn);
					if(R != null) 
						return R;
